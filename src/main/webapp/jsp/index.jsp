...
<div class="am-cf admin-main">
  ...
  <div class="admin-content">
    ...
    <ul class="am-avg-sm-1 am-avg-md-4 am-margin am-padding am-text-center admin-content-list ">
      <li><a href="${pageContext.request.contextPath}/project/check/project" class="am-text-success"><span class="am-icon-btn am-icon-puzzle-piece"></span><br/>项目总数<br/>${count.project }</a></li>
      <li><a href="#" class="am-text-warning"><span class="am-icon-btn am-icon-tasks"></span><br/>作业总数<br/>${count.work }</a></li>
      <li><a href="#" class="am-text-danger"><span class="am-icon-btn am-icon-user-md"></span><br/>教师总数<br/>${count.teacher }</a></li>
      <li><a href="#" class="am-text-secondary"><span class="am-icon-btn am-icon-graduation-cap"></span><br/>学生总数<br/>${count.student }</a></li>
    </ul>

    <div class="am-g" style="margin-bottom: 250px;">
      <div class="am-u-sm-12">
        <table class="am-table am-table-bd am-table-striped admin-content-table">
          <thead>
          <tr>
            <th>#</th><th>项目所有者</th><th>项目名称</th><th>发布时间</th><th>管理</th>
          </tr>
          </thead>
          <tbody>
          <c:forEach var="obj" items="${indexProjectList}" varStatus="status">
          <tr>
          	<td>${status.index+1}</td><td>${obj.userName}</td><td><a href="#">${obj.title}</a></td><td><span class="am-badge am-badge-success">${obj.time }</span></td>
            <td>
                <button class="am-btn am-btn-default am-btn-xs am-dropdown-toggle"  onclick="download('${obj.attachment}')"><span class="am-icon-download">下载附件</span></button>
            </td>
          </tr>
          </c:forEach>
          </tbody>
...
function download(location){
	...
	{
		layer.msg("没有附件", {icon: 5});
		return;
	}
	setTimeout(function () {
		window.location = "${pageContext.request.contextPath}/download?location="+location;
    }, 1);
}
</script>
</body>
