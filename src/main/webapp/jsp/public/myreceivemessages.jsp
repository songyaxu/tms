    ...
    <div class="am-g">
      <div class="am-u-sm-12">
          <table class="am-table am-table-hover table-main">
            <thead>
              <tr>
                </th><th class="table-id">#</th><th class="table-title">内容</th><th class="table-type">发件人</th><th class="table-author">状态</th><th class="table-date">发布时间</th><th class="table-set">操作</th>
              </tr>
          </thead>
          <tbody>
            <c:forEach var="obj" items="${receiveList}" varStatus="status">
          <tr style="${obj.state eq 0 ? 'background-color:#FFB1B0;':''}">
          	<td>${status.index+1}</td><td>${obj.content}</td><td><a href="#">${obj.sendId}</a></td><td><c:if test="${obj.state==0}">未读</c:if><c:if test="${obj.state==1}">已读</c:if></td><td>${obj.sendTime }</td>
            <td>
          ...
          <div class="am-cf">
  共 ${receivePage.totalCount } 条记录
  <div class="am-fr">
    <ul class="am-pagination">
      <li class="${receivePage.hasPrePage eq true ? '':'am-disabled'}"><a href="<c:if test="${receivePage.hasPrePage eq true}">javascript:pageTurn(${receivePage.currentPage-1})</c:if>">«</a></li>
	  <c:if test="${receivePage.hasPrePage==true}">
			<li><a href="javascript:pageTurn(${receivePage.currentPage-1})">${receivePage.currentPage-1}</a></li>
	  </c:if>
      <li class="am-active"><a href="#">${receivePage.currentPage }</a></li>
      <c:if test="${receivePage.currentPage+1<=receivePage.totalPage}">
			<li><a href="javascript:pageTurn(${receivePage.currentPage+1})">${receivePage.currentPage+1}</a></li>
	  </c:if>
	  <c:if test="${receivePage.currentPage+2<=receivePage.totalPage}">
			<li><a href="javascript:pageTurn(${receivePage.currentPage+2})">${receivePage.currentPage+2}</a></li>
	  </c:if>
	  <c:if test="${receivePage.currentPage+3<=receivePage.totalPage}">
			<li><a href="javascript:pageTurn(${receivePage.currentPage+3})">${receivePage.currentPage+3}</a></li>
	  </c:if>
      <li class="${receivePage.hasNextPage eq true ? '':'am-disabled'}"><a href="<c:if test="${receivePage.hasNextPage==true}">javascript:pageTurn(${receivePage.currentPage+1})</c:if>">»</a></li>
    </ul>
  </div>
</div>
      </div>
...
function pageTurn(page){
	setTimeout(function () {
		window.location = "${pageContext.request.contextPath}/message/myreceiver/"+page;
    }, 0);
}
function detail(id){
	setTimeout(function () {
