...
<div class="am-cf admin-main">
  ...
  <div class="admin-content">
    ...
    <div class="am-cf am-padding">
      <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">查看小组</strong> / <small>Group</small></div>
    </div>
    ...
    <div class="am-g">
      <div class="am-u-sm-12">
          <table class="am-table am-table-hover table-main">
            <thead>
              <tr>
                </th><th class="table-id">#</th><th class="table-title">组名</th><th class="table-type">小组成员</th><th class="table-author">组长</th><th class="table-set">操作</th>
              </tr>
          </thead>
          <tbody>
            <c:forEach var="obj" items="${group}" varStatus="status">
          <tr>
          	<td>${status.index+1}</td><td>${obj.title}</td><td>
          		<c:forEach var="itemu" items="${obj.userList}" varStatus="index">
          			${index.index>0 ?"、":""} ${itemu.name} 
          		</c:forEach>
          	</td><td><a href="#">${obj.masterId }</a></td>
            <td>
                <div class="am-btn-toolbar">
                  <div class="am-btn-group am-btn-group-xs">
                  	<button class="am-btn am-btn-default am-btn-xs am-text-default" onclick="javascript:join(${obj.projectId},${obj.id});"><span class="am-icon-check-circle-o"></span>加入该组</button>
                  </div>
                </div>
              </td>
          </tr>
...
function join(projectId,id){
	layer.confirm('加入该组后将无法修改小组您确定要继续加入该组吗？', {
		  btn: ['确定','取消']
		}, function(){
			$.post("${pageContext.request.contextPath}/group/join/"+projectId+"/"+id,
	    		    function(data){
	        		if(data.code==1){
