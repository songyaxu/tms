...
<div class="admin-content">
  ...
  <div class="am-cf am-padding">
    <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">发布项目</strong> / <small>publish</small></div>
  </div>
      ...
      <div class="am-tab-panel am-fade" id="tab3">
        <form class="am-form">
          <div class="am-g am-margin-top-sm">
            ...
            <div class="am-u-sm-10">
              <textarea rows="10" placeholder="" id="groupMaster"></textarea>
            </div>
          </div>
           <div class="am-g am-margin-top">
            <div class="am-u-sm-2 am-text-right">
            ...
            <div class="am-u-sm-4">
              <input type="text" class="am-input-sm" id="maxGroup">
            </div>
            <div class="am-u-sm-6" ></div>
          </div>
          <div class="am-g am-margin-top">
            ...
            <div class="am-u-sm-4">
              <input type="text" class="am-input-sm" id="groupMaxMember">
            </div>
            <div class="am-u-sm-6" ></div>
          </div>
        </form>
...
function publish(){
	...
	var groupMaster=$("#groupMaster").val();
	var groupProject=$('input[name=options]:checked').val();
	var maxGroup=$("#maxGroup").val();
	var groupMaxMember=$("#groupMaxMember").val();
	if(title==="")
	{
	...
	if(groupMaster==="")
	{
		layer.msg("请输入组长需求描述",{icon: 5});
		return;
	}
	if(groupProject==null||groupProject==0)
	{
		if(maxGroup===""){
			layer.msg("请输入最大组数",{icon: 5});
			return;
		}
		if(groupMaxMember===""){
			layer.msg("请输入小组最大成员数",{icon: 5});
			return;
		}	
	}
   	$.get("${pageContext.request.contextPath}/project/publish",
   		{title:title,description:description,attachment:attachment,groupMaster:groupMaster,maxGroup:maxGroup,groupMaxMember:groupMaxMember,groupProject:groupProject},
	    function(data){
   		if(data.code==1){
   			layer.msg(data.message,{icon: 1});
   			clr();
...
function clr(){
	...
	$("#label1").removeClass("am-active");
	$("#label2").removeClass("am-active");
}
</script>
</body>
</html>
